...
export async function createTask(userId: string, input: CreateTaskInput): Promise<Task> {
  ...
  // Story 3.1 Task 8 (Correct Course 2026-09-05): plant voortaan direct ALLE sessies tot
  // en met het doelmoment (i.p.v. 1 sessie), elk binnen een echt beschikbare-tijd-blok
  // (AD-10) — zie sprint-change-proposal-2026-09-05.md. `planSessionSlots` doet zelf de
  // live Calendar-blok-opzoekingen; er is nog geen `task.id` om als excludeTaskId mee te
  // geven (deze taak bestaat nog niet), dus een lege lijst.
  let avgAvailableMinutes: number
  let doelmoment: string
  let sessionSlots: Awaited<ReturnType<typeof planSessionSlots>>
  try {
    ...
    sessionSlots = await planSessionSlots(userId, today, doelmoment, totalMinutes, input.defaultSessionDuration)
  } catch (fout) {
    console.error(`[tasks] Kon beschikbare tijd niet live ophalen uit de gekoppelde Calendar-agenda voor user ${userId}:`, fout)
    throw fout
  }

  // Task-insert, Session-inserts (Task 8: N i.p.v. 1) en Subtask-inserts lopen atomair in
  ...
  const { task, sessions } = await createTaskAndSessions({
    task: {
      userId,
    ...
    sessionSlots: sessionSlots.map(slot => ({ startsAt: slot.startsAt, plannedMinutes: slot.plannedMinutes })),
  ...
  // is al zelf-bewakend op kleur + write-scope (Story 2.3 AC #4). Task 8: nu voor élke
  // betrokken datum, niet meer voor slechts 1 sessie.
  //
  ...
  const distinctDates = new Set(sessions.map(session => session.startsAt.slice(0, 10)))
  for (const date of distinctDates) {
    try {
      await syncHomeworkBlocksForDate(userId, date)
    } catch (fout) {
      console.error(`[tasks] Kon huiswerk-Calendar-blokken niet synchroniseren na aanmaken van taak ${task.id} (${date}):`, fout)
    }
  }
