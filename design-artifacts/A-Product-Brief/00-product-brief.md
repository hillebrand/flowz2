...
3. Read existing materials if the user has them (check `../_progress/wds-project-outline.yaml`)

**Existing materials for this project** (see outline `existing_materials`): a complete BMM brief, PRD, and architecture spine already exist at `_bmad-output/planning-artifacts/`. Use these as grounding, don't re-derive from scratch — but still confirm with the user rather than assuming the WDS brief is a copy-paste of the BMM one.
...
| # | Document | Status |
|---|----------|--------|
| — | Pitch (Alignment & Signoff) | Done — `pitch.md` |
