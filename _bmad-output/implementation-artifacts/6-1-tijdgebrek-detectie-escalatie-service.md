...
| 2026-09-03 | **Herzien voor AD-10 (Correct Course 2026-09-02)** — herziene AC #2/#3 in `epics.md`: niveau 2 ("tijd verruimen") heeft niet langer een direct accept-effect (Flowz mag/kan de gekoppelde Google Calendar-agenda niet namens Evelien aanpassen); wordt voortaan puur instructief getoond, met een nieuwe "recheck"-flow als Evelien de aanpassing zelf in Calendar heeft doorgevoerd. **Aanleiding:** Story 3.1's Task 7 (2026-09-03, AD-10-rework van de scheduling-engine) had niveau 2's generatie als tussenstap al volledig uitgeschakeld — die AD-10-rework maakte `applyVerruimen`'s oude schrijf-naar-`AvailableTimeException` een no-op, en het "wat betekent tijd verruimen nu" was op dat moment expliciet doorverwezen naar déze story. **Nu heringevoerd met de juiste, herziene semantiek:** `shortfall.ts`'s `generateShortfallRecommendations` genereert niveau 2 weer, met instructieve omschrijvingstekst ("Verruim {dag} met minstens {duur} in je beschikbare-tijd-agenda") i.p.v. de oude "van X naar Y"-tekst die een direct Flowz-effect suggereerde. `apply-recommendation.ts`'s `applyVerruimen` blijft een expliciete fout gooien (nu met een bijgewerkte, definitieve boodschap i.p.v. "tijdelijk") — er is voor deze tier bewust geen accept-pad meer, alleen de recheck-actie (Story 6.2's UI-zorg). **Geen nieuwe "recheck"-functie nodig:** `detectShortfallForDate`/`generateShortfallRecommendations` lazen al vóór deze wijziging live uit de gekoppelde Calendar zonder enige cache/tussenstaat (Task 7's rework) — een "recheck" ís simpelweg dezelfde functies nogmaals aanroepen voor dezelfde datum; geen apart mechanisme te bouwen binnen déze story's service-only-scope. Typecheck/build slagen, opnieuw gedeployed, en live bevestigd met een echte testtaak (geen gekoppelde agenda → tekort van 40 min → niveau 2 toont de nieuwe instructieve tekst, escaleert correct door naar niveau 3 omdat 30 min niet genoeg is). Testtaak opgeruimd. Status blijft `done`. |
...
**AD-10-herziening (2026-09-03) — gewijzigd:**
- `server/domain/scheduling/shortfall.ts` (niveau 2 heringevoerd met instructieve tekst, `VERRUIMEN_STEP_MINUTES` teruggezet)
- `server/domain/scheduling/apply-recommendation.ts` (`applyVerruimen`'s foutmelding definitief gemaakt, was "tijdelijk")

**AD-10-herziening — live gedeployed:** dev-stage op `flowz.fyi`. Geen migratie nodig.
