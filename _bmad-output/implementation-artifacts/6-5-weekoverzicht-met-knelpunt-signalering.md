...
| 2026-09-04 | **Recheck-patroon alsnog geïmplementeerd (aangevinkt als "nog te implementeren" sinds de AD-10-herziening van 2026-09-02, opgepakt tijdens Story 6.7's opvolgwerk).** `week-day-suggestion-accept-button` toonde ook voor een niveau-"verruimen"-suggestie altijd "Accepteren", terwijl die tier sinds AD-10 geen accept-effect meer heeft — een klik daarop faalde met een generieke 500 (`applyVerruimen` gooit bewust een `Error`). Nieuw, bewust dun `server/api/week/[date]/suggestion/recheck.post.ts` (spiegelt Story 6.2's `.../recheck.post.ts`: geen mutatie, alleen `buildWeekDay` opnieuw aanroepen — `detectShortfallForDate` erin leest toch al live uit de Calendar). `app/pages/week/index.vue` toont voor `tier === 'verruimen'` nu `week-day-suggestion-recheck-button` ("Ik heb dit aangepast — controleer opnieuw") i.p.v. Accepteren, met dezelfde "hele week opnieuw ophalen"-afhandeling als `accepteren`. `typecheck`/`build` schoon, opnieuw gedeployed, live geverifieerd op de dev-stage: geen 500 meer, kaart blijft correct staan zonder foutmelding. |
