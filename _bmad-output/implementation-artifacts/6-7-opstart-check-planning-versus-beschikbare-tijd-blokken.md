...
- [ ] **Task 4: Verificatie**
  ...
  - [ ] Live: testscenario met een gekoppelde beschikbare-tijd-agenda + een testtaak die precies past. Verwijder/verkort daarna (buiten Flowz om, direct in Google Calendar) het blok voor de geplande dag zodat er een tekort ontstaat maar er een andere dag met ruimte binnen de deadline is → Home laden → sessie stil verplaatst (DB bevestigt nieuwe datum), geen enkele zichtbare melding, taakkaart toont de bijgewerkte planning.
  - [ ] Live: zelfde opzet maar zonder alternatieve dag met ruimte (bv. deadline morgen, geen enkele dag met genoeg capaciteit) → Home laden → navigatie naar `/herstel/tekort-oplossen`, tekort zichtbaar met de bestaande escalatieketen.
  - [ ] Live: gebruiker zonder gekoppelde beschikbare-tijd-agenda → Home laden → verwijzing naar `/instellingen` zichtbaar, geen crash, geen poging tot herplannen.
  - [ ] Live: bevestig dat `/agendaconflict/aanpassen`, `GET /api/availability/conflicts`, `PATCH /api/availability/exceptions/{date}` nu allemaal 404 geven (routes zijn verwijderd) en dat Home niet meer naar `ConflictModal` verwijst.
  - [ ] Geen secrets/placeholder-waarden in code/commits. Testdata (taak/sessie, tijdelijke Calendar-blok-wijziging) na verificatie opgeruimd/hersteld.
...
| 2026-09-04 | Implementatie Tasks 0-3 afgerond: sanering (10 vervallen/kapotte bestanden verwijderd via `git rm`, `app/pages/index.vue` opgeschoond en uitgebreid met `home-availability-notice`), `runStartupReplanCheck` (`server/domain/scheduling/startup-check.ts`), `GET /api/scheduling/startup-check` + `shared/types/startup-check.d.ts`, Home-integratie (niet-blokkerende fetch + `watch`-gestuurde navigatie naar `/herstel/tekort-oplossen`). `npx nuxt typecheck` en `npx nuxt build` beide schoon. Gedeployed naar de dev-stage (`npx sst deploy --stage dev`, `flowz.fyi`) — geslaagd. Live-verificatie (Task 4's overige subtaken) kan niet vanuit deze sessie: alle routes zitten achter de globale sessie-middleware (curl zonder een echte, ingelogde sessie geeft overal 401, ook op nu-verwijderde routes — geen betrouwbare 404-bevestiging mogelijk zonder een echte login). Hillebrand voert de live-verificatie zelf uit; status blijft `in-progress` tot die subtaken bevestigd zijn. |
...
- **Live-verificatie kon niet vanuit deze sessie:** de app is volledig authenticatie-gated (globale sessie-middleware) — een onaangemelde `curl` krijgt overal 401 terug, inclusief op de nu-verwijderde routes, dus 404-bevestiging vereist een echte ingelogde sessie. Hillebrand heeft aangegeven dit zelf live te verifiëren op `flowz.fyi` (dev-stage, net gedeployed).
