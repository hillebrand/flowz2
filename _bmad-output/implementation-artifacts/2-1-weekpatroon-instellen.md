...
| 2026-07-31 | **Na `done`, op expliciete productbeslissing van Hillebrand:** een plafond van 24 uur (`MAX_MINUTES_PER_DAY`, `shared/utils/availability.ts`) toegevoegd aan `updateWeekPatternDay`'s `increase`-pad, wijkt af van de UX-spec's "geen bovengrens". Typecheck en build slagen; **nog niet gedeployed/live geverifieerd** — de AWS-sessie verliep tijdens het werk aan Story 2.2's code review, en Hillebrand koos ervoor dit voorlopig niet opnieuw te doen. Zie ook Story 2.2's Change Log voor dezelfde wijziging aan de exceptie-kant. |
