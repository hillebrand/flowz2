...
### De TOCTOU-race, alsnog opgepakt (2026-08-17/18) — twee aparte bugs, niet één

Deze story's eigen Dev Notes noemden ooit een bewust geaccepteerde TOCTOU-race in de stapelings-offset-berekening, met de aantekening "oppakken zodra de eerste échte replan-trigger-story dit daadwerkelijk gelijktijdig kan laten gebeuren." Die trigger was allang afgegaan (`recalculateTaskPlanning` wordt inmiddels vanuit minstens acht plekken aangeroepen), maar stond nergens in `deferred-work.md` gelogd totdat een bredere audit (op verzoek van Hillebrand, aanleiding: een onbereikbaar-scherm-omissie bij Story 2.1) het alsnog naar boven haalde. Het oppakken bleek twee volledig aparte problemen te bevatten, pas onderscheiden na uitgebreid live-onderzoek:

**Bug 1 — een échte TOCTOU-race.** Twee gelijktijdige aanroepen kunnen elkaars lees-dan-schrijf-sectie overlappen. Drie opeenvolgende pogingen waren nodig om dit daadwerkelijk te sluiten tegen déze Turso/`@libsql/client/web`-verbinding:
1. Een multi-statement-transactie (`getDb().transaction(...)`, "write"-modus/`BEGIN IMMEDIATE`) — zelfde patroon als het al langer bestaande, destijds "empirisch geverifieerd" geachte `server/data/availability.ts`. **Faalde**: een live concurrency-test (4 gelijktijdige aanroepen, opzettelijk verschillende sessieduren om toeval wiskundig uit te sluiten) toonde aan dat alle vier elkaars staat lazen vóórdat ook maar één schreef.
2. Een single-statement optimistic-concurrency-guard (de bezetting herberekend als scalar-subquery binnen de `UPDATE`'s eigen `WHERE`-clausule — in theorie atomair, ongeacht transactiegedrag). **Faalde eveneens**, empirisch bevestigd met dezelfde testopzet, zelfs na het losstaand verifiëren dat de gegenereerde SQL (`.toSQL()`) syntactisch correct was.
3. Turso's eigen documentatie over "Concurrent Writes" (op verzoek van Hillebrand geraadpleegd vóórdat een derde poging werd gebouwd) beschrijft de **standaardmodus** (zonder `PRAGMA journal_mode='mvcc'`, wat dit project nooit heeft aangezet) als "één schrijfverbinding tegelijk, transacties wachten in een wachtrij" — wat, letterlijk gelezen, poging 1 en 2 hadden moeten laten werken. Waarom dat in de praktijk niet zo bleek (tegen déze specifieke HTTP-gebaseerde verbinding) is niet volledig doorgrond. **Oplossing die wél empirisch werkt:** een expliciete, database-afgedwongen lock-rij (`sessionPlacementLocks`, nieuwe tabel, `UNIQUE` op user+datum) rond de hele kritieke sectie — bevestigd via server-side CloudWatch-logging die een nette CONFLICT→wachten→VERKREGEN-sequentie liet zien.

**Bug 2 — een structurele plaatsingsfout, volledig los van concurrency.** Ontdekt tíjdens het testen van de lock-tabel-fix: dezelfde overlap bleef optreden bij een **strikt sequentiële** test (twee `/replan`-aanroepen ná elkaar, geen enkele gelijktijdigheid). Oorzaak: de stapelings-formule (`anker + som van ieders duur, behalve die van mezelf`) garandeert wiskundig dat élke taak op hetzelfde eindpunt uitkomt (`anker + som van alle betrokken taken samen`) zodra twee of meer taken die dezelfde dag delen, ná elkaar herberekend worden — puur optel-wiskunde (`A_eind = anker + B_duur + A_duur`, `B_eind = anker + A_duur + B_duur`, en optellen is commutatief), ongeacht volgorde of gelijktijdigheid. Dit trof specifiek `resolve-conflict.post.ts` (Story 6.6), de enige aanroeper die `recalculateTaskPlanning` in een lus over meerdere taken aanroept. **Oplossing:** `recalculateTaskPlanning`/`placeSessionWithStackingOffset` accepteren nu een lijst van uit te sluiten taak-id's (niet langer één) — `resolve-conflict.post.ts` geeft per lus-iteratie de nog-niet-verwerkte batchgenoten mee, zodat elke taak in de batch ook hén uitsluit van "hoeveel is er al bezet." De drie single-task-aanroepers (`update-task.ts`, `replan.ts`, `session-heartbeat-fallback.ts`) blijven ongewijzigd — hun gedrag was, en is, correct: één taak die op zichzelf herberekend wordt (geen andere taak die in dezelfde operatie ook beweegt) plaatst zich terecht "na iedereen anders."

Live geverifieerd na de definitieve fix: (a) 4 gelijktijdige aanroepen met verschillende duren → geen overlap; (b) de échte `resolve-conflict`-flow met twee taken die door hetzelfde conflict naar dezelfde nieuwe dag verplaatst worden → correct na elkaar gestapeld, geen overlap. Alle testdata (incl. drie tijdelijke debug-logging-rondes met `console.error`, telkens vóór de volgende poging weer verwijderd) opgeruimd.
...
| 2026-08-18 | **De destijds bewust geaccepteerde TOCTOU-race alsnog opgepakt**, op verzoek van Hillebrand na een bredere audit die het (nooit in `deferred-work.md` gelogde) punt naar boven haalde. Bleek twee aparte bugs te zijn — zie Dev Notes voor het volledige onderzoek (3 mislukte pogingen vóór de werkende oplossing, incl. geraadpleegde Turso-documentatie en CloudWatch-bewijs). **Fix 1** (échte race): nieuwe `sessionPlacementLocks`-tabel (migratie `0015_eager_magdalene.sql`) + `acquireSessionPlacementLock`/`releaseSessionPlacementLock` in `server/data/tasks.ts`, rond de kritieke sectie van `placeSessionWithStackingOffset`. **Fix 2** (structurele plaatsingsfout, los van concurrency, pas ontdekt tíjdens het testen van fix 1): `placeSessionWithStackingOffset`/`recalculateTaskPlanning` accepteren nu een lijst uit te sluiten taak-id's i.p.v. één; `resolve-conflict.post.ts` (Story 6.6) geeft per lus-iteratie de nog-niet-verwerkte batchgenoten mee. Live geverifieerd: 4 gelijktijdige aanroepen zonder overlap, én de échte `resolve-conflict`-flow met twee taken die naar dezelfde nieuwe dag verplaatst worden zonder overlap. `deferred-work.md` bijgewerkt. |
| 2026-09-03 | **AD-10-check op `recalculate.ts` (geen code gewijzigd) + nieuwe-AC-status verduidelijkt.** Gecontroleerd of `recalculateTaskPlanning` (net als Story 3.4's `sortByVolgorde`) automatisch meelift met Story 3.1's Task 7-rework: `recalculate.ts` roept `averageDailyAvailableMinutes`/`calculateDoelmoment`/`findSessionDate` aan — dezelfde functies die Task 7 al naar de live Calendar-bron herbouwde, met ongewijzigde handtekening. Gerichte grep bevestigt geen enkele resterende `getOrCreateWeekPattern`/`getExceptionForDate`/`AvailableTimePattern`/`AvailableTimeException`-referentie in `recalculate.ts`/`server/data/tasks.ts`. **De tweede, nog openstaande implementation-note ("app-open als triggermoment") is bewust géén losse taak in déze story** — `sprint-status.yaml` noteerde zelf al "volgt bij 6-7": `recalculateTaskPlanning` bestaat al als herbruikbare, kant-en-klare domain-functie (dat is precies deze story's eigen AC #1, "herbruikbaar als het gedeelde herberekenings-mechanisme voor Epic 4/5/6's replan-triggers"); wat ontbreekt is niet meer functie-logica hier, maar de daadwerkelijke aanroeper — Story 6.7's herziene "opstart-check" (vervangt de oude agendaconflict-detectie) roept 'm bij het openen van de app aan. Nu al een losstaande app-open-route in déze story bouwen zou 6.7's eigen ontwerp (wat er precies gebeurt bij meerdere te herberekenen taken, wanneer te escaleren) vooruitlopen zonder dat ontwerp te kennen. Geen wijziging aan `sprint-status.yaml`'s note nodig — die beschrijft de situatie al correct. |
...
**Toegevoegd/gewijzigd bij het oppakken van de TOCTOU-race (2026-08-18):**
- `server/data/schema.ts` (gewijzigd — nieuwe `sessionPlacementLocks`-tabel)
- `server/data/migrations/0015_eager_magdalene.sql` (nieuw)
- `server/data/tasks.ts` (gewijzigd — `placeSessionWithStackingOffset` herschreven met lock + `excludeTaskIds`-lijst i.p.v. één id; `acquireSessionPlacementLock`/`releaseSessionPlacementLock` toegevoegd)
- `server/domain/scheduling/recalculate.ts` (gewijzigd — `additionalExcludeTaskIds`-parameter)
- `server/api/availability/day/[date]/resolve-conflict.post.ts` (gewijzigd — geeft nog-niet-verwerkte batchgenoten door aan `recalculateTaskPlanning`)
- Tijdelijke `console.error('[DEBUG-RACE...]')`-logregels (drie onderzoeksrondes) zijn allemaal weer verwijderd vóór de definitieve versie; horen niet bij deze File List.
