...
- 2026-09-04, later dezelfde dag (verzoek Hillebrand): **de samenvoeging zelf (déze story's kernidee) is teruggedraaid.** Met de titel nu vak+taak-specifiek zou een gedeeld blok van meerdere aaneengesloten taken een onhandige kommagescheiden titel krijgen — Hillebrand wil expliciet één blok per taak, altijd, ook zonder tussenliggend bezet agenda-item. `groupSessionsIntoBlocks` (met zijn bezet-item-splitsingslogica, `getBlockingEventsForDate`, en de `isBlockingEvent`/`overlapInterval`/`getTodayEvents`-imports die daarvoor nodig waren) is vervangen door een simpele `sessionsToBlocks`: elke sessie wordt 1-op-1 haar eigen `ComputedBlock`. `matchBlocks` (nearest-start-time-koppeling aan bestaande DB-blokken) blijft ongewijzigd — werkt net zo goed 1-op-1. Zelf-helend voor al bestaande, vóór deze wijziging samengevoegde blokken (geen migratie nodig, zelfde precedent als eerdere AD-10-sanering). `typecheck`/`build` schoon, opnieuw gedeployed, live geverifieerd: twee aaneengesloten taken op dezelfde dag (16:00-16:20 en 16:20-16:40, geen bezet item ertussen) kregen nu daadwerkelijk twee losse Calendar-events met elk hun eigen titel, i.p.v. één samengevoegd blok. Testtaken + Calendar-events opgeruimd.
