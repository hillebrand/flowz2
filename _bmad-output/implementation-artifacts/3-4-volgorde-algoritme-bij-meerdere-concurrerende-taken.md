...
| 2026-09-03 | **AD-10-implementation-note geverifieerd en afgesloten, geen code gewijzigd.** `sprint-status.yaml` noteerde sinds de Correct Course (2026-09-02) "zelfde als 3-1, nog te herbouwen" bij deze story. Bij nazicht bleek `sortByVolgorde` (`ordering.ts`) `averageDailyAvailableMinutes` nooit zelf aan te roepen — die waarde komt als parameter `avgDailyMinutes` binnen, opgehaald door de aanroepers (`server/api/home/plan.get.ts`, `server/domain/scheduling/shortfall.ts`). Story 3.1's Task 7 (2026-09-03, zie die story's Change Log) herbouwde `averageDailyAvailableMinutes` al naar de live Calendar-bron met **ongewijzigde functiehandtekening** — precies om deze reden: elke downstream-consument, inclusief déze story's volgorde-algoritme, liftte daardoor automatisch mee, zonder dat `ordering.ts`/`getTasksWithSessionOnDate` zelf één regel hoefden te wijzigen. Geverifieerd met een gerichte grep op `getOrCreateWeekPattern`/`getExceptionForDate`/`AvailableTimePattern`/`AvailableTimeException` in `ordering.ts`, `server/data/tasks.ts` en `plan.get.ts` — geen enkele treffer. Geen typecheck/build/deploy nodig (geen bestand gewijzigd). Implementation-note in `sprint-status.yaml` verwijderd. |
