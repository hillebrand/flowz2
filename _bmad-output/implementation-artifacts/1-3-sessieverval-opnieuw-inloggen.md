...
**Toegevoegd/gewijzigd bij het oppakken van de deferred punten (2026-08-17):**
- `app/plugins/sessie-verval.client.ts` (nieuw — globale `$fetch`-`onResponseError`-interceptor)
- `app/components/TaakFormulier.vue` (gewijzigd — `sessionExpired`-state, `skipSessieVervalRedirect`-coördinatievlag, aangepaste 401-afhandeling in de save-flow, link naar `/inloggen` in `taak-save-error`)

## Change Log

| Datum | Wijziging |
| --- | --- |
| 2026-08-17 | **Deferred punten opgepakt op verzoek van Hillebrand**, na afronding van Epic 6 (alle epics/stories nu `done`). Beide triggers waren afgegaan: Epic 2/3 hebben allang geauthenticeerde API-calls, en Story 5.3's `TaakFormulier.vue` is inmiddels het eerste échte formulier. (1) **AC #1's API-helft**: nieuw `app/plugins/sessie-verval.client.ts` — een globale `$fetch`-`onResponseError`-interceptor die élke 401 project-breed naar `/inloggen` leidt, i.p.v. het losse per-pagina `is401()`-patroon dat her en der al bestond (dat blijft ongewijzigd staan, onschadelijk redundant). (2) **AC #1's "onopgeslagen data"-clausule**: `TaakFormulier.vue`'s opslaan-flow toont bij een 401 tijdens het opslaan nu een expliciete melding ("Je sessie is verlopen...") met een handmatige link naar `/inloggen`, i.p.v. automatisch weg te navigeren — de ingevulde data blijft daardoor gewoon zichtbaar. Een nieuwe gedeelde `useState<boolean>('skip-sessie-verval-redirect', ...)`-vlag laat het formulier de globale interceptor tijdelijk overrulen tijdens het opslaan. `typecheck`/`build` beide schoon, gedeployed, live geverifieerd: Home laadt ongewijzigd, een echte testtaak werd succesvol opgeslagen via de nu gewrapte `$fetch` (geen regressie). De daadwerkelijke 401-redirect-paden zijn niet apart live geforceerd — dat vereist een sessie met een echt verlopen/ongeldige cookie, wat zonder een riskante cookie-forgery-truc niet reproduceerbaar was; vertrouwd op code-inspectie + typecheck, consistent met hoe Story 6.7's per-dag-foutafscherming eerder ook niet apart geforceerd is. `deferred-work.md` bijgewerkt. |
