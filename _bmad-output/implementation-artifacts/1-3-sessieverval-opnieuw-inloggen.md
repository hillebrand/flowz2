...
- **Neveneffect bevestigd:** `/inloggen` geeft géén `Set-Cookie` meer, omdat dat pad in de publieke-padlijst van de middleware staat en de sessie daar dus nooit wordt aangeraakt. Dat lost en passant de cookie-churn op die in de review van Story 1.2 als los punt genoteerd stond (elke anonieme response kreeg een `Set-Cookie`, wat CloudFront-caching onmogelijk maakte). Paden die de sessie wél aanraken zetten de cookie correct met `Expires` +7 dagen.
