...
| 2026-09-04 | **Bug gevonden en gefixt tijdens Story 6.7's opvolgwerk** (AD-10-herziening, 2026-09-02, voegde `controlerenOpnieuw`/de recheck-route al toe, maar wire'de de knop nooit aan het sjabloon): `shortfall-recommendation-accept-button` verscheen nog altijd op "Tijd verruimen"-kaarten, terwijl die tier sinds AD-10 geen accept-effect meer heeft (`applyVerruimen` gooit bewust een `Error`) — een klik daarop faalde dus altijd met een generieke 500. Gepatcht: de template toont voor `tier === 'verruimen'` nu `shortfall-recommendation-recheck-button` ("Ik heb dit aangepast — controleer opnieuw", roept de al-bestaande `controlerenOpnieuw` aan) i.p.v. Accepteren, en verbergt Afwijzen voor die tier (UX-spec, letterlijk). `typecheck`/`build` schoon, opnieuw gedeployed, live geverifieerd op de dev-stage: geen 500 meer, kaart blijft correct staan zonder foutmelding bij "nog niet aangepast". |
